...
void USB_Send_Data(u8* data_buffer, uint16_t size)
{
    UserToPMABufferCopy(data_buffer, ENDP2_TXADDR, size);
    SetEPTxCount(ENDP2, size);
    SetEPTxValid(ENDP2);
    
    /* Write the descriptor through the endpoint */    
//    USB_SIL_Write(EP2_IN, (uint8_t*) data_buffer, size);  
//    SetEPTxValid(ENDP2);
}
